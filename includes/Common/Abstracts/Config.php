		...
		$defaultSettings = apply_filters(
			...
			array(
				'woocommerce_iyzico_settings' => array(
					...
					'payment_checkout_value' => __( 'Thank you for your order, please enter your card information in the payment form below to pay with iyzico checkout.',
						'woocommerce-iyzico' ),
					'title'                  => __( 'Pay with Bank/Debit Card', 'woocommerce-iyzico' ),
					'button_text'            => __( 'Pay With Card', 'woocommerce-iyzico' ),
					'description'            => __( 'Pay with your credit card or debit card via iyzico.',
						'woocommerce-iyzico' ),
				...
				'woocommerce_pwi_settings'    => array(
					...
					'description' => __( "Your money safe with iyzico! Store your iyzico card and enjoy one-click payment. All your transactions under the iyzico Buyer Protection guarantee. Get live support 24/7.",
						'woocommerce-iyzico' ),
				),
			)
		);

		return array_key_exists( $this->optionsTableKey,
			(array) $defaultSettings ) ? $defaultSettings[ $this->optionsTableKey ] : [];
	}
